...
def inconsistency(x, y):
        # return 1 / 2 * (1 - x) * (1 - y) +  x * y
    return   (1 - x) * (1 - y) + 1 / 2 * x * y

@ROTATED_DETECTORS.register_module()
class FrequenceDetV3(RotatedBaseDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FrequenceDetV3, self).__init__(init_cfg)
        if pretrained:
    ...
    def extract_feat(self, img):
        ...
        processed_feats_ir = list()
        processed_feats_vis = list()
        
        for feat, feat_ir in zip(x_rgb, x_ir):
           
            # 使用 DWTTransformerattentionlayer 进行特征处理
            processed_feat,processed_feat_vis,processed_feat_ir = self.dwt_transformer_attention(feat, feat_ir)       
            processed_feats.append(processed_feat)
            processed_feats_vis.append(processed_feat_vis)
            processed_feats_ir.append(processed_feat_ir)

        refine_ir_features = list()
        for i in range(len(x_ir)):
            fefine_ir_feature = processed_feats_ir[i] + x_ir[i]
            refine_ir_features.append(fefine_ir_feature)

        incons_feature = list()
        for i in range(len(x_rgb)):
            incons_feature.append(inconsistency(processed_feats_vis[i], processed_feats_ir[i]))


        vis_weight = [None for _ in range(5)]
        for i in range(5):
            vis_weight[i] = incons_feature[i] * x_rgb[i]
 
        final_features = list()           
        for i in range(len(x_ir)):
            final_feature = vis_weight[i] + refine_ir_features[i]
            final_features.append(final_feature)
            
        # if self.with_neck:
        #     outputs = self.neck(final_features)
